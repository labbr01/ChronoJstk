...
namespace ChronoJstk.Chat
{
    ...
    public sealed class SenderBluetoothService // : ISenderBluetoothService
    {
        ...
        public async Task<IList<DeviceBt>> GetDevices()
        {
            // for not block the UI it will run in a different threat
            var task = Task.Run(() =>
            {
                var devices = new List<DeviceBt>();
                using (var bluetoothClient = new BluetoothClient())
                {
                    ...
                    {
                        devices.Add(new DeviceBt(array[i]));
                    }
                }
                return devices;
            });
        ...
        public async Task<bool> Send(DeviceBt device, string content)
        {
            if (device == null)
            {
